        ...
        for i in range(len(self.armies)):
            if len(self.armies[i].units) + len(self.armies[i].units_merged) < 10:
                ...
                if army == "A":
                    ...
                    narmy = Army('images/blueArmy.png', self.identifier, 1, 0, 0, [unit], [unit], 800, False, 1.5)
                    self.armies.append(narmy)
                elif army == "B":
                    unit = Unit(0, gridSize-1, gridSize-1, 0, 5, 'images/redUnit.png')
                    narmy = Army('images/redArmy.png', self.identifier, 2, gridSize-1, gridSize-1, [unit], [unit], 800, False, 1.5)
                    self.armies.append(narmy)
                self.morale += 0.5
                self.identifier += 1
        self.money -= 1000
